...
addpath(genpath('./IMR_DA/IMR-vanilla/functions'));
addpath(genpath('./IMR_DA/IMR-vanilla/src'));

%% ---- Parameters -------------------------------------------------------

model  = 'neoHook';     % 'neoHook' | 'linkv' | 'sls' | 'nhzen' | 'fung'
G      = 0;          % shear modulus (Pa)
mu     = 0.001;          % viscosity (Pa·s)
G1     = Inf;           % dashpot modulus (Inf → KV limit)
...
P_gas_init    = 226;    % initial non-condensible gas pressure (Pa·P_inf units)
...
fprintf('Running funIMRsolver: model=%s, G=%.0f Pa, mu=%.4f Pa·s, R0=%.1f µm\n', ...
    model, G, mu, R0*1e6);
tic;
[t, R, U, P, S, T, C, Tm, tdel, Tdel, Cdel, Tau, pf] = funIMRsolver( ...
    model, matprop, tspan, R0, NT, NTM, ...
    Pext_type, Pext_Amp_Freq, disptime, Tgrad, Tmgrad, Cgrad, Dim, comp, REq, IMRsolver_RelTolX,5e6);
fprintf('Done in %.1f s\n', toc);

% Dimensional time in µs
t_us = t * 1e6;
...
P_inf = Pmt(19);
plot(t, pf, 'k', 'LineWidth',1.5);
xlabel('Time (µs)'); ylabel('P (Pa)');
